...
string pathName = "../images/digits.png";
int SZ = 20;
...
Mat deskew(Mat& img) {
    ...
    /*
    //resize digitImg
    Mat resized_img, resize_imgOut;
    resize(img, resized_img, Size(100, 100), INTER_LINEAR);
    imshow("digitImg", resized_img);
    resize(imgOut, resize_imgOut, Size(100, 100), INTER_LINEAR);
    imshow("digitImgOut", resize_imgOut);
    waitKey(5);
    */
...
void loadTrainTestLabel(string& pathName, vector<Mat>& trainCells, vector<Mat>& testCells, vector<int>& trainLabels, vector<int>& testLabels)
{
    ...
    for (int i = 0; i < img.rows; i = i + SZ)
    {
        for (int j = 0; j < img.cols; j = j + SZ)
        {
            ...
            /*
            //resize digitImg
            Mat resized_digitImg;
            resize(digitImg, resized_digitImg, Size(100, 100), INTER_LINEAR);
            imshow("digitImg", resized_digitImg);
            waitKey(5);
            */
...
void SVMevaluate(Mat& testResponse, float& count, float& accuracy, vector<int>& testLabels, vector<Mat>& testCells)
{
    for (int i = 0; i < testResponse.rows; i++)
    {
        ...
        if (testResponse.at<float>(i, 0) == testLabels[i])
            ...
            //resize digitImg
            Mat resized_img;
            resize(testCells.at(i), resized_img, Size(100, 100), INTER_LINEAR);
            imshow("digitImg", resized_img);
            waitKey(5000);
        }
    }
    accuracy = (count / testResponse.rows) * 100;
    imshow("output", testResponse);
}
...
int main()
{
    ...
    float accuracy = 0;
    getSVMParams(model);
    SVMevaluate(testResponse, count, accuracy, testLabels, testCells);
